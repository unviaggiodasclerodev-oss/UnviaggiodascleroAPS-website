...
<template>
  <header :class="[
    ...
    isScrolled
      ? 'bg-stone-900/95 backdrop-blur-md border-b border-white/10 py-3'
      : 'bg-transparent py-5'
  ]">
    <!-- Scroll progress bar -->
    ...
    <div class="section-pad flex items-center justify-between">
      <a href="#hero" @click.prevent="scrollTo('#hero')">
        <img src="/logo.png" alt="Un Viaggio da Sclero APS" class="h-10 w-auto object-contain" />
      </a>
      ...
      <nav class="hidden md:flex items-center gap-8">
        ...
        <button @click="toggle"
          :class="['w-9 h-9 rounded-full flex items-center justify-center border transition-all hover:border-accent cursor-pointer',
            isScrolled ? 'border-white/20 text-white/60' : 'border-white/30 text-white/70']"
      ...
      <div class="md:hidden flex items-center gap-3">
        <button @click="toggle" :class="['p-1.5', isScrolled ? 'text-white' : 'text-white']">
    ...
    <Transition name="slide">
      <div v-if="mobileOpen"
        class="md:hidden bg-stone-900 border-t border-white/10 section-pad py-6 flex flex-col gap-5">
        <a v-for="link in navLinks" :key="link.label"
          :href="link.href" @click.prevent="scrollTo(link.href)"
          class="text-white/80 text-base font-medium hover:text-accent transition-colors">
          {{ link.label }}
        </a>
        <a href="#contatti" @click.prevent="scrollTo('#contatti')"
